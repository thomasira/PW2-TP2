...
abstract class Crud extends PDO {
    ...
    public function read($order = null) {
        $join = "";
        if(is_array($this->target)) $this->target = implode(", ", $this->target);
        if(isset($this->tablesMg))
            foreach($this->tablesMg as $tableMg) 
                $join .= "LEFT OUTER JOIN $tableMg ON " . $tableMg . "_id = $tableMg" . ".id";
        $sql = "SELECT $this->target FROM $this->table $join ORDER BY id $order";
    ...
    public function readId($value) {
        ...
        if(isset($this->tablesMg)) 
            foreach($this->tablesMg as $tableMg) 
                $join .= "LEFT OUTER JOIN $tableMg ON " . $tableMg . "_id = $tableMg" . ".id";
        $sql = "SELECT $this->target FROM $this->table $join WHERE $this->table.$this->primaryKey = :$this->primaryKey";
        $query = $this->prepare($sql);
